...
class Graphics
{
	...
	// Draw_cloud description:
        ...
        // Cloud shape using a custom point
        void draw_cloud(int cloudnumber, Point middle);
    // Cloud shape using the cloud's point
	...
	// Draw_Sun Description:
    	void draw_sun(Point middle, double radius);
	// Draw_Wave Description
        void draw_wave(double height, double width, double period, Point middle, Color wave);
    //  Draw_Tornado
        void draw_Tornado(double height, double width, double period, Point middle, Color tornado);
...
void Graphics::draw_boat(Point middle, double height, double base, double width_mast, double depth, double degrees, Color ofboat, Color ofmast)
{
    ...
    */
    ...
    for (double i=xmin; i<xmax; i+=0.2){
        for (int k=middle.y(); k<vect[g].y(); k++){
            rotate(i,k);
        }
        g++;
    }
    ...
    int p=0;
    for (double i=middle.x(); i<middle.x()+width_mast; i+=0.2){
        for (double k=middle.y()-height/2; k>slant[p].y(); k-=0.2){
            rotate(i,k);
        }
        p++;
    }
...
void Graphics::set_rain(int cloudnumber, double percentage, int sizethreshold, double speed, Color color0, double rainthreshold)
{
    ...
    if (widh*heigh>rainthreshold){ 
        ...
        int xmax=midpoint.x()+widh/2.5;
        int xmin=midpoint.x()-widh/2.5;
        int ymax=midpoint.y()+heigh/2.5;
        int ymin=midpoint.y()-heigh/2.5;
...
void Graphics::draw_cloud(int cloudnumber, Point middle)
{
    if (cloudnumber>=sizeof(cloudlocation)){
    }
    else{
...
void Graphics::draw_cloud(int cloudnumber)
{
    if (cloudnumber>=sizeof(cloudlocation)){
    }
    else{
        ...
        Color ost=cloudlocation[cloudnumber].get_color();
        int red=ost.get_r();
        int green=ost.get_g();
        int blue=ost.get_b();
        double percentage=80.00*area/(px*py);
        Point middle=cloudlocation[cloudnumber].get_middle();
...
void Graphics::draw_sun(Point middle, double radius)
{
    double x=middle.x();
    double y=middle.y();
...
void Graphics::draw_wave(double height, double width, double period, Point middle, Color wave)
{
    ...
    {
        Point os=Point(1.0*i,1.0*height*sin(k*i+ k*width+2.00*M_PI*period)+400.00);
        venter.push_back(os);
    }
